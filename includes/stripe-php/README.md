...
```php
$stripe = new \DRStripe\StripeClient('sk_test_BQokikJOvBiI2HlWgH4olfQ2');
...
$curl = new \DRStripe\HttpClient\CurlClient();
$curl->setTimeout(10); // default is \DRStripe\HttpClient\CurlClient::DEFAULT_TIMEOUT
$curl->setConnectTimeout(5); // default is \DRStripe\HttpClient\CurlClient::DEFAULT_CONNECT_TIMEOUT

echo $curl->getTimeout(); // 10
echo $curl->getConnectTimeout(); // 5

// tell Stripe to use the tweaked client
\DRStripe\ApiRequestor::setHttpClient($curl);
...
```php
// set up your tweaked Curl client
$curl = new \DRStripe\HttpClient\CurlClient([CURLOPT_PROXY => 'proxy.local:80']);
// tell Stripe to use the tweaked client
\DRStripe\ApiRequestor::setHttpClient($curl);
```
...
```php
\DRStripe\Stripe::setLogger($logger);
```
...
```php
$curl = new \DRStripe\HttpClient\CurlClient([CURLOPT_SSLVERSION => CURL_SSLVERSION_TLSv1]);
\DRStripe\ApiRequestor::setHttpClient($curl);
```
...
```php
\DRStripe\Stripe::setCABundlePath("path/to/ca/bundle");
```
...
```php
\DRStripe\Stripe::setMaxNetworkRetries(2);
```
...
```php
\DRStripe\Stripe::setEnableTelemetry(false);
```
...
```php
\DRStripe\Stripe::setAppInfo("MyAwesomePlugin", "1.2.34", "https://myawesomeplugin.info");
```
