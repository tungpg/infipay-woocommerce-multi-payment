...
* [#1241](https://github.com/stripe/stripe-php/pull/1241) Add generic parameter to \DRStripe\Collection usages
...
* [#1155](https://github.com/stripe/stripe-php/pull/1155) Add streaming methods to Service infra
  ...
  * `\DRStripe\RequestOptions::parse` now clones its input if it is already a `RequestOptions` object, to prevent accidental mutation.
...
## 7.25.0 - 2020-02-14
* [#879](https://github.com/stripe/stripe-php/pull/879) Make `\DRStripe\Collection` implement `\Countable`
...
-   Use `\DRStripe\<class>::class` constant instead of strings ([#643](https://github.com/stripe/stripe-php/pull/643))
...
-   [#539](https://github.com/stripe/stripe-php/pull/539) Fix methods on `\DRStripe\PaymentIntent` to properly pass arguments to the API.

## 6.19.4 - 2018-10-11

-   [#534](https://github.com/stripe/stripe-php/pull/534) Fix PSR-4 autoloading for `\DRStripe\FileUpload` class alias

## 6.19.3 - 2018-10-09

-   [#530](https://github.com/stripe/stripe-php/pull/530) Add constants for `flow` (`FLOW_*`), `status` (`STATUS_*`) and `usage` (`USAGE_*`) on `\DRStripe\Source`
...
-   [#520](https://github.com/stripe/stripe-php/pull/520) Rename `\DRStripe\FileUpload` to `\DRStripe\File`
...
-   [#502](https://github.com/stripe/stripe-php/pull/502) Add `isDeleted()` method to `\DRStripe\StripeObject`

## 6.12.0 - 2018-07-28

-   [#501](https://github.com/stripe/stripe-php/pull/501) Add support for scheduled query runs (`\DRStripe\Sigma\ScheduledQueryRun`) for Sigma
...
-   [#498](https://github.com/stripe/stripe-php/pull/498) Internal improvements to the `\DRStripe\ApiResource.classUrl()` method
...
-   [#493](https://github.com/stripe/stripe-php/pull/493) Add PHPDoc for `auto_advance` on `\DRStripe\Invoice`
...
-   [#481](https://github.com/stripe/stripe-php/pull/481) Add new `\DRStripe\Discount` and `\DRStripe\OrderItem` classes, add more PHPDoc describing object attributes
...
-   [#479](https://github.com/stripe/stripe-php/pull/479) Fix unnecessary traits on `\DRStripe\InvoiceLineItem`

## 6.7.2 - 2018-05-28

-   [#471](https://github.com/stripe/stripe-php/pull/471) Add `OBJECT_NAME` constant to all API resource classes, add `\DRStripe\InvoiceLineItem` class
...
-   [#458](https://github.com/stripe/stripe-php/pull/458) Add PHPDoc for `account` on `\DRStripe\Event`
...
-   [#453](https://github.com/stripe/stripe-php/pull/453) Add constants for `reason` (`REASON_*`) and `status` (`STATUS_*`) on `\DRStripe\Dispute`

## 6.3.2 - 2018-02-27

-   [#452](https://github.com/stripe/stripe-php/pull/452) Add PHPDoc for `amount_paid` and `amount_remaining` on `\DRStripe\Invoice`

## 6.3.1 - 2018-02-26

-   [#443](https://github.com/stripe/stripe-php/pull/443) Add event types as constants to `\DRStripe\Event` class
...
-   [#442](https://github.com/stripe/stripe-php/pull/442) Fix PHPDoc for `\DRStripe\Error\SignatureVerification`
...
*   `\DRStripe\AttachedObject` no longer exists. Attributes that used to be instances of `\DRStripe\AttachedObject` (such as `metadata`) are now instances of `\DRStripe\StripeObject`.

-   Attributes that used to be PHP arrays (such as `legal_entity->additional_owners` on `\DRStripe\Account` instances) are now instances of `\DRStripe\StripeObject`, except when they are empty. `\DRStripe\StripeObject` has array semantics so this should not be an issue unless you are actively checking types.

*   `\DRStripe\Collection` now derives from `\DRStripe\StripeObject` rather than from `\DRStripe\ApiResource`.
...
-   Add `\DRStripe\Stripe::setLogger()` to support an external PSR-3 compatible logger
...
-   Rename `\DRStripe\Object` to `\DRStripe\StripeObject` (PHP 7 compatibility)
