...
class OAuthService extends \DRStripe\Service\AbstractService
{
    /**
     ...
     *
     * @return \DRStripe\StripeObject the object returned by Stripe's Connect API
     */
    ...
    public function authorizeUrl($params = null, $opts = null)
    {
        ...
        $query = \DRStripe\Util\Util::encodeParameters($params);
     ...
     * @param null|array $opts
     *
     * @throws \DRStripe\Exception\OAuth\OAuthErrorException if the request fails
     *
    ...
    private function _getClientId($params = null)
    {
        ...
        if (null === $clientId) {
            $msg = 'No client_id provided. (HINT: set your client_id using '
              . '`new \DRStripe\StripeClient([clientId => <CLIENT-ID>
     ...
     *
     * @return \DRStripe\Util\RequestOptions
     */
    private function _parseOpts($opts)
    {
        if (\is_array($opts)) {
            if (\array_key_exists('connect_base', $opts)) {
                ...
                // \DRStripe\Service\OAuthService from \DRStripe\OAuth, where `connect_base`
                // was the name of the parameter that behaves as `api_base` does here.
                throw new \DRStripe\Exception\InvalidArgumentException('Use `api_base`, not `connect_base`');
            }
        }

        return \DRStripe\Util\RequestOptions::parse($opts);
    }
