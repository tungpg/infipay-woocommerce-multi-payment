...
try {
    ...
    $db = new PaymentServerDB();
    
    $paymentProcess = $db->getPaymentClientbyCode($data['payment_code']);
    if(($paymentProcess)){
        $db->deletePaymentClientsbycode($data['payment_code']);
    }
    $paymentProcess = $db->createPaymentClients($data);
